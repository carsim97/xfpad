"""The cyclic ordering of the PAI prototypes an optimiser would choose.
...
This is NOT the assignment X-FPAD uses. In the framework a material is bound
to its sector by its label index, which is what makes the map reproducible
across trainings. The ordering computed here is consumed by
`scripts/permutation_stability.py` as `cosopt`, one of the challenger layouts
the attribution is tested against: if the anchors an unseen PAI receives were
an artefact of the arbitrary label order, the similarity-optimal order is the
one most likely to expose it.
